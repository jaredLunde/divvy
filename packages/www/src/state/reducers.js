...
export const company = (state = {}, action) => {
  switch (action.type) {
    case actions.ADD_COMPANY:
      return {...parseCreatedAt(action.company), status: action.status}
  ...
  switch (action.type) {
    case actions.ADD_SHAREHOLDER:
      if (action.status === actions.Api.DONE)
        return {
           members: [...state.members, parseCreatedAt(action.shareholder)],
      ...
      if (action.status === actions.Api.DONE) {
        ...
        for (let i = 0; i < members.length; i++) {
          ...
          if (member.id === action.shareholder.id)
            members[i] = {...member, ...parseCreatedAt(action.shareholder)}
        }
    ...
    case actions.RECEIVE_SHAREHOLDERS:
      return {members: action.shareholders.map(parseCreatedAt), status: action.status}
...
export const shares = (state = [], action) => {
  switch (action.type) {
    case actions.ADD_SHARE:
      if (action.status === actions.Api.DONE)
        return {
          members: [...state.members, parseCreatedAt(action.share)],
          status: action.status
        }
      return {...state, status: action.status}
      ...
      if (action.status === actions.Api.DONE) {
        ...
        for (let i = 0; i < members.length; i++) {
          ...
          if (member.id === action.share.id)
            members[i] = {...member, ...parseCreatedAt(action.share)}
        }
    ...
    case actions.RECEIVE_SHARES:
      return {members: action.shares.map(parseCreatedAt), status: action.status}
